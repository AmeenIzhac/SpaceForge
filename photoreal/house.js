...
const poses = [];
{
  ...
  let s = 0, yaw, rate = 0, phase = "walk", phaseT = 0, stopIdx = 0, panBase = 0;
  const h0 = new THREE.Vector3().subVectors(poly[1], poly[0]).normalize();
  yaw = Math.atan2(-h0.x, -h0.z);
  const headingAt = (ss) => {
    const p = atS(ss), q = atS(Math.min(ss + 0.45, TOTAL));
    const d = new THREE.Vector3().subVectors(q, p);
    return d.lengthSq() > 1e-8 ? Math.atan2(-d.x, -d.z) : yaw;
  };
  let guard = 0;
  while (guard++ < 20000) {
    ...
    if (phase === "walk") {
      ...
      targetYaw = panBase + st.amp * Math.sin(2 * Math.PI * u) * Math.sin(Math.PI * u);
      if (phaseT >= st.dur) { phase = "turn"; phaseT = 0; }
    } else { // turn in place toward the resumed path
      targetYaw = headingAt(s + 0.02);
      phaseT += dt;
      if (Math.abs(angdiff(targetYaw, yaw)) < 0.12 || phaseT > 4) {
        phase = "walk";
        stopIdx++;
      }
    }
    ...
    yaw += rate;
    const p = atS(s);
    poses.push([p.x, p.z, yaw, s, sv]);
  }
  for (let i = 0; i < 20; i++) poses.push(poses[poses.length - 1]);
}
...
window.renderFrame = (i) => {
  ...
  const [x, z, yaw, s, sv] = poses[k];
  const bobY = 0.010 * sv * Math.sin(2 * Math.PI * s / 0.72);
  const sway = 0.005 * sv * Math.sin(2 * Math.PI * s / 1.44);
  camera.position.set(x + Math.cos(yaw) * sway, EYE + bobY, z - Math.sin(yaw) * sway);
  camera.rotation.set(-0.03, yaw, 0);
  composer.render();
  return true;
};
